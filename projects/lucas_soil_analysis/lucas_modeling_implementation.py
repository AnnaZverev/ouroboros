...
DATA_PATH = repo_root / "LUCAS_SOIL_2018_AlphaEarth_Embeddings_Wheat_clean.csv"
...
def load_data():
    ...
    # Convert non-numeric "ND" and similar to NaN
    for col in df.columns:
        df[col] = pd.to_numeric(df[col], errors="coerce")

    # Keep rows with all required targets for multi-task
    ...
    print(f"CaCO3-only additional samples: {len(df_caco3) - len(df_multi[mask_caco3])}")
...
def approach2_moe(df_multi):
    print("\n"+"="*80)
    print("APPROACH 2: Target-Specific MoE (Feature Selection)")
    print("="*80)
...
def approach3_hybrid(df_multi, df_caco3):
    print("\n"+"="*80)
    print("APPROACH 3: Hybrid with Geospatial Covariates")
    print("="*80)
    ...
    if lat_col and lon_col:
        ...
        X_enriched["Lat_abs"] = np.abs(lat - lat.mean())
        X_enriched["Lon_abs"] = np.abs(lon - lon.mean())
    ...
    # CaCO3 strategy: enforce embeddings-only model
    ...
    for t in TARGET_COLS:
        if t == "CaCO3":
            # CaCO3 predictions use different test set
            artifacts["y_pred"][t] = preds_ca.tolist()
            artifacts["caco3_test_indices"] = X_ca_te_idx.tolist()
        else:
            artifacts["y_pred"][t] = models[t].predict(X_te_sel if t!="CaCO3" else X_test_enr).tolist()
...
def generate_comparison_table(results1, results2, results3):
    ...
    rows = []
    for target in TARGET_COLS:
        r1 = results1.get(target, {}).get("r2")
        r2 = results2.get(target, {}).get("r2")
        r3 = results3.get(target, {}).get("r2")
        rmse1 = results1.get(target, {}).get("rmse")
        rmse2 = results2.get(target, {}).get("rmse")
        rmse3 = results3.get(target, {}).get("rmse")
        rows.append([
            target,
...
4. CaCO3 strategy: Instead of a separate sub-model, use a two-stage model where CaCO3 predictions are informed by neighboring points' geospatial similarity (KNN on spatial covariates) plus embeddings.
