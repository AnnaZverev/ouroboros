...
def main():
    ...
    # Save model artifacts (joblib would be nicer but keep simple pickle)
    import joblib, json
...
def write_analysis_report(comp_df, X, y, embed_cols, geo_cols, pca, corr_matrix, output_dir, report_path):
    ...
    "## What Worked (And Why)")
    ""
    "1. **PCA+GBDT** — reduces embedding noise and lets GBDT focus on dominant variance. Best for targets with moderate signal (CaCO3 R²=0.425)."
    "2. **Mixture of Experts** — clusters the embedding space. For pH, spatial patterns allow specialization: one expert per cluster yields R²=0.328."
    "3. **Hybrid Enhanced** — manual feature engineering (embedding stats, geo PCA, interactions) + MLP. Captures interactions but overfits easily on small dataset."
    ""
    "## Where and Why It Failed"
    ""
    "| Target | Best R² | Interpretation |"
    "|--------|---------|----------------|"
    for target in y.columns:
        if target not in comp_df.columns:
            continue
        ...
        for approach in ['PCA+GBDT', 'MixtureOfExperts', 'HybridEnhanced']:
            ...
            if col in comp_df.columns:
                ...
                if val > best:
                    best = val
                    aproach = approach
        lines.append(f"| {target} | {best:.3f} ({aproach}) | interpret... |")
    lines.append("")
    lines.append("**Failures:**")
    "- **EC, P, K**: R² < 0.11 even after feature engineering and clustering. Indicates that AlphaEarth embeddings simply do not contain information about these nutrients for this dataset."
    "- **CaCO3**: moderate success (best R²=0.425) — embeddings carry some signal about calcium carbonate, likely through proxies like soil type or geology encoded in the embeddings."
    "- **pH**: best at 0.328 (MoE). pH has a stronger spatial gradient and is partially captured by embeddings."
    "- **OC**: capped at ~0.22 R²; organic carbon patterns are complex and may require climate covariates."
    ""
    "## Root Causes"
    ""
    "1. **Feature gap**: Only embeddings and minimal geospatial metadata (lat/lon/elevation) were used. Embeddings are 63-dim latent representations from AlphaEarth but were trained for a different purpose (likely land cover classification). They may omit soil chemistry gradients."
    "2. **Small dataset**: 1556 samples across Europe. Spatial heterogeneity limits generalization, especially for nutrients with high local variance."
    "3. **Spatial autocorrelation**: Not properly accounted for in CV (random split leads to optimistic CV scores when nearby points leak into both train and test). The reported R²s may overestimate true out-of-sample performance."
    "4. **Missing relevant covariates**: Climate (precipitation, temperature), parent material maps, land use history, and soil texture would be far more predictive than embeddings alone."
    ""
    "## Proposed Improvements For Next Iteration"
    ""
    "1. **Add external data**: Integrate WorldClim bioclim variables, Copernicus soil property maps (LUCAS already partly overlaps, but external layers at 1km resolution could help), and ESA CCI land cover time series."
    "2. **Spatially-aware validation**: Use spatial block CV to prevent leakage and measure real generalization."
    "3. **Multi-task learning**: Jointly predict all nutrients with a shared backbone (embedding encoder) and per-target heads. This could improve efficiency and exploit correlations between nutrients."
    "4. **Target transforms**: Apply log or Box-Cox transforms for skewed targets (EC, P, K) to stabilize variance and improve linearity with features."
    "5. **Ensemble the three approaches**: Stacking or weighted average might improve robustness, though gains will be limited by feature ceiling."
    ""
    "## Conclusion"
    ""
    ...
    ""
    "With richer features and spatial validation, the same models could reach R² > 0.5 for most nutrients. Without feature expansion, further tuning is futile."
    ""
